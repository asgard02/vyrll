...
function ProjetsContent() {
  ...
  // Rafraîchir les clips quand on revient sur l’onglet (ex. après création sur le dashboard)
  useEffect(() => {
    if (projectTab !== "clips") return;
    const onVisible = () => {
  ...
  // Poll pour la progression des jobs en cours (projets)
  const inProgressIds = clipJobs
    .filter((j) => j.status === "pending" || j.status === "processing")
    .map((j) => j.id)
    .join(",");
  useEffect(() => {
    if (projectTab !== "clips" || !inProgressIds) return;
    const poll = async () => {
    ...
    poll();
    const t = setInterval(poll, 6000); // 6s — aligné avec dashboard/clips
    return () => clearInterval(t);
  }, [projectTab, inProgressIds]);
              ...
              clipsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
                  ...
                  <div className="font-[family-name:var(--font-syne)] font-bold text-xl text-white mb-2">
                    {profile?.plan === "free" ? "Clips réservés aux abonnés Pro" : "Aucun projet clips"}
                  </div>
                  <p className="font-mono text-sm text-zinc-500 mb-6 max-w-sm">
                    {profile?.plan === "free"
                      ? "Passe à Pro pour générer des clips viraux à partir de tes vidéos."
                      : "Génère des clips viraux pour les voir ici."}
                  </p>
                  <Link
                    href={profile?.plan === "free" ? "/plans" : "/dashboard"}
